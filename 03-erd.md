...
### 2.1. Sơ đồ Conceptual ERD (Mức khái niệm)

Sơ đồ thể hiện mối quan hệ nghiệp vụ ở mức tổng quan giữa các thực thể cốt lõi trong hệ thống:

```mermaid
erDiagram
    ROLE ||--o{ USER : "gán cho"
    USER ||--o{ WATCHLIST : "theo dõi"
    STOCK ||--o{ WATCHLIST : "được thêm vào"
    MARKET ||--o{ STOCK : "chứa"
    INDUSTRY ||--o{ STOCK : "thuộc về"
    
    STOCK ||--o{ FACT-MARKET-PRICE : "có thông tin giá"
    STOCK ||--o{ FACT-FINANCIAL-STATEMENT : "có báo cáo tài chính"
    REPORT-PERIOD ||--o{ FACT-FINANCIAL-STATEMENT : "thuộc kỳ"
    TIME ||--o{ FACT-MARKET-PRICE : "ghi nhận lúc"
    DATA-SOURCE ||--o{ FACT-MARKET-PRICE : "cung cấp bởi"
    
    CRAWL-JOB ||--o{ CRAWL-LOG : "chạy sinh ra"
    CRAWL-LOG ||--o{ CRAWL-LOG-DETAIL : "chứa chi tiết"
    STOCK ||--o{ CRAWL-LOG-DETAIL : "được ghi log cho"
```

### 2.2. Sơ đồ Logical ERD (Mức logic)

Sơ đồ chi tiết chỉ ra các khóa chính (PK), khóa ngoại (FK) và liên kết cấu trúc dữ liệu theo Star schema:

```mermaid
erDiagram
    %% Dimensions
    dim_markets {
        ObjectId market_id PK
        string code UK
        string name
        string country
        string timezone
    ...
    dim_industries {
        ObjectId industry_id PK
        string industry_name
        string sector_name
    }
    dim_stocks {
        ObjectId stock_id PK
    ...
    dim_report_periods {
        ObjectId report_period_id PK
        int fiscal_year
        int fiscal_quarter
        string period_name
    }
    dim_data_sources {
        ObjectId data_source_id PK
        string name
        string type
        string status
    }
    ...
    %% Facts
    fact_market_prices {
        ObjectId market_price_id PK
        ObjectId stock_id FK
        ObjectId market_id FK
        ObjectId industry_id FK
        ObjectId data_source_id FK
        int time_id FK
        double open_price
        double close_price
        double volume
    ...
    fact_financial_statements {
        ObjectId financial_statement_id PK
        ObjectId stock_id FK
        ObjectId report_period_id FK
        ObjectId data_source_id FK
        double net_revenue
        double profit_after_tax
        double total_assets
        double equity
    }
    ...
    %% Relationships
    dim_markets ||--o{ dim_stocks : "contains"
    dim_industries ||--o{ dim_stocks : "categorizes"
    
    dim_stocks ||--o{ fact_market_prices : "has_price"
    dim_markets ||--o{ fact_market_prices : "listed_on"
    dim_industries ||--o{ fact_market_prices : "industry_stats"
    dim_data_sources ||--o{ fact_market_prices : "sourced_from"
    dim_time ||--o{ fact_market_prices : "recorded_at"
    
    dim_stocks ||--o{ fact_financial_statements : "has_financials"
    dim_report_periods ||--o{ fact_financial_statements : "for_period"
    dim_data_sources ||--o{ fact_financial_statements : "financials_from"

    roles ||--o{ users : "assigned_to"
    users ||--o{ watchlists : "owns"
    dim_stocks ||--o{ watchlists : "watched"
```
...
| `country`        | Quốc gia                             |
| `timezone`       | Múi giờ                              |
| `market_src_url` | URL nguồn dữ liệu của sàn            |
...
| `stock_id`              | Khóa ngoại từ `dim_stocks`         |
| `trade_stats_url`       | URL của thống kê mã cổ phiếu       |
| `market_price_data_url` | URL giá thị trường của cổ phiếu    |
| `financial_data_url`    | URL báo cáo tài chính của cổ phiếu |
...
- Phục vụ dashboard đánh giá sức khỏe thị trường (HOSE, UPCOME,...).
...
# 8. Sơ đồ Vật lý & Chiến lược Lập chỉ mục (Physical Schema & Indexing)

Do đặc thù của cơ sở dữ liệu NoSQL (MongoDB), thiết kế vật lý tập trung vào cấu trúc lưu trữ Document lồng nhau (Nested Documents) và các chỉ mục (Indexes) để tối ưu hiệu năng truy vấn Dashboard:

### 8.1. Lập chỉ mục vật lý (Indexes)
1. **fact_market_prices**:
   - `index({ stock_id: 1, time_id: -1 }, { unique: true })` -> Tối ưu hóa việc tải biểu đồ nến lịch sử cho một mã cụ thể theo thứ tự thời gian giảm dần.
   - `index({ time_id: -1, market_id: 1 })` -> Tối ưu hóa tải dữ liệu bảng giá/dashboard toàn bộ sàn HOSE theo ngày gần nhất.
2. **fact_financial_statements**:
   - `index({ stock_id: 1, report_period_id: -1 }, { unique: true })` -> Tối ưu hóa biểu đồ tài chính so sánh theo quý.
3. **watchlists**:
   - `index({ user_id: 1, stock_id: 1 }, { unique: true })` -> Tối ưu hóa kiểm tra và hiển thị Watchlist riêng của từng người dùng.
4. **dim_stocks**:
   - `index({ symbol: 1 }, { unique: true })` -> Tìm kiếm nhanh mã cổ phiếu từ thanh Search.

### 8.2. Ví dụ Document vật lý (`fact_market_prices`)
```json
{
  "_id": "ObjectId('60d5ec4b1f8e2c3a50d21051')",
  "stock_id": "ObjectId('60d5ec4b1f8e2c3a50d21001')",
  "market_id": "ObjectId('60d5ec4b1f8e2c3a50d21002')",
  "industry_id": "ObjectId('60d5ec4b1f8e2c3a50d21003')",
  "data_source_id": "ObjectId('60d5ec4b1f8e2c3a50d21004')",
  "time_id": 20260607,
  "open_price": 135.5,
  "close_price": 137.5,
  "volume": 1250000.0,
  "bid_volume": 450000.0,
  "ask_volume": 300000.0,
  "foreign_buy": 150000.0,
  "foreign_sell": 80000.0,
  "foreign_net": 70000.0,
  "market_cap": 120500000000.0,
  "eps": 4250.0,
  "pe": 32.35,
  "forward_pe": 29.5,
  "bvps": 42500.0,
  ...
  "crawled_at": "2026-06-07T15:30:00.000Z",
  "created_at": "2026-06-07T15:35:00.000Z"
}
```
